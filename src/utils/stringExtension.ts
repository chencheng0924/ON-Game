...
String.prototype.getlogoSvg = function (darkTheme: boolean): string {
    let fileName = this;
    return new URL(`../assets/logo/${darkTheme ? 'logo-white' : 'logo-dark'}.png`, import.meta.url).href
}
...
String.prototype.getIcon = function (imgType: string): string {
    let fileName = this;
    return new URL(`../assets/icon/${fileName}.${imgType}`, import.meta.url).href
}
